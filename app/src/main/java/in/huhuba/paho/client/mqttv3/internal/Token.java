...
import in.huhuba.paho.client.mqttv3.internal.wire.MqttSuback;
...
public class Token {
	private static final String TAG = LogUtils.makeLogTag(Token.class);
	...
	protected MqttWireMessage waitForResponse(long timeout) throws MqttException {
		...
		synchronized (responseLock) {
            ...
            LOGD(TAG, methodName+" 400");

			while (!this.completed) {
				if (this.exception == null) {
					try {
						//@TRACE 408=key={0} wait max={1}
                        LOGD(TAG, methodName+" 408");
